...
Trying to structure objects intelligently ran into constant frustration in the form of unexplained object behaviors (such as self-disabling objects), or the 'intended' method for parenting network objects to eachother just flat-out not working when implemented as prescribed; all of which taking up about 70% of development time and sanity.

The resulting dead-ends and inconsistent structures in the code are fixable, but without intelligible documentation and usage instructions for FishNet, that effort also feels condemned to another series of exhausting workarounds for problems for which an obtuse or poorly documented "intended" feature exists.
